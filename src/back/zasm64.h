...
struct zasm
{
	...
	{
		vasm+=rf::vstr(rskey(c_mov64),rskey(c_rcx),rsoptr(c_comma),rstr(*((int64*)(&dval))));
		vasm+=rf::vstr(rskey(c_push),rskey(c_rcx));
	}

	static rbool add_asm_rbp(const tsh& sh,tfunc& tfi)
	{
		rbuf<tasm> vasm;
		tasm item;
		item.pos=tfi.pos;
		...
		{
			item.vstr=rf::vstr(rskey(c_sub),rskey(c_rsp),
				rsoptr(c_comma),rstr(size));
			vasm+=item;
		}
		rbool b_use_rbp=use_rbp(sh,tfi.vasm);
		rbool protect_ebx=(tfi.name.get_left()==r_char('~')&&
			!yfind::is_empty_struct_type(sh,tfi.ptci->name)&&
			!tfi.vsent.empty()||tfi.is_cfunc);
		if(b_use_rbp)
		{
			item.vstr=rf::vstr(rskey(c_push),rskey(c_rbp));
			vasm+=item;
			item.vstr=rf::vstr(rskey(c_mov64),rskey(c_rbp),
				rsoptr(c_comma),rskey(c_rsp));
			vasm+=item;
		}
		if(protect_ebx)
		{
			item.vstr=rf::vstr(rskey(c_push),rskey(c_rbx));
			vasm+=item;
		}
		if(tfi.is_cfunc)
		{
			//todo 如果ebx、esi、edi未修改则不用保护
			item.vstr=rf::vstr(rskey(c_push),rskey(c_rsi));
			vasm+=item;
			item.vstr=rf::vstr(rskey(c_push),rskey(c_rdi));
			vasm+=item;
		}
		vasm+=tfi.vasm;
		item.pos=tfi.pos;
		if(tfi.is_cfunc)
		{
			...
			item.vstr.push(rskey(c_rdi));
			vasm.push(item);

			item.vstr.clear();
			item.vstr.push(rskey(c_pop));
			item.vstr.push(rskey(c_rsi));
			vasm.push(item);
		}
		if(protect_ebx)
		{
	...
	static rbool use_rbp(const tsh& sh,const rbuf<tasm>& vasm)
	{
		ifn(tconf::c_op_rbp)
		{
			return true;
		...
		tfunc& tfi,int level=0)
	{
		...
		if(src.vword.count()==1)
		{
			...
			elif(src.vword[0].is_cpoint())
			{
				...
				{
					vasm+=rf::vstr(rskey(c_mov64),
						rskey(c_rbx),rsoptr(c_comma),
						src.vword[0].val);
				}
				else
				{
		...
		if(ptdi!=null)
		{
			...
			{
				vasm+=rf::vstr(rskey(c_lea),rskey(c_rsi),
					rsoptr(c_comma),rsoptr(c_mbk_l),
					rskey(c_rbp),rsoptr(c_plus),
					rstr(ptdi->off),rsoptr(c_mbk_r));
				//这里还可以优化
				ifn(add_rsi(sh,ptdi->type,vasm,src))
				{
					rserror("");
					return false;
				}
				vasm+=rf::vstr(rskey(c_mov64),rskey(c_rbx),
					rsoptr(c_comma),rsoptr(c_mbk_l),
					rskey(c_rsi),rsoptr(c_mbk_r));
		...
		tfunc& tfi,int level)
	{
		...
		{
			vasm+=rf::vstr(rskey(c_mov64),rskey(c_rsi),
				rsoptr(c_comma),rskey(c_rsp));
			int cur=vasm.count();
			ifn(add_rsi(sh,retval.type,vasm,src))
			{
				rserror("");
				return false;
			}
		...
		vasm+=rf::vstr(rskey(c_add),rskey(c_rsp),rsoptr(c_comma),size);
		return true;
	}
		...
		tfunc& tfi,int level=0)
	{
		...
		if(src.vword.get(1).val==rskey(c_pcall))
		{
			...
			size=yfind::get_ceil_space(retval);
			vasm+=rf::vstr(rskey(c_sub),rskey(c_rsp),
				rsoptr(c_comma),size);
			...
			{
				vasm+=rf::vstr(rskey(c_call),rskey(c_rbx));
			}
			else
			{
				rserror("");
		...
		size=yfind::get_ceil_space(retval);
		vasm+=rf::vstr(rskey(c_sub),rskey(c_rsp),rsoptr(c_comma),size);
		for(int i=vsent.count()-1;i>=0;i--)
		{
		...
		tfunc& tfi,int level)
	{
			...
			yfind::is_class(sh,src_in.vword.get(1).val))
		{
			...
			//释放栈
			size=yfind::get_ceil_space(retval);
			vasm+=rf::vstr(rskey(c_add),rskey(c_rsp),
				rsoptr(c_comma),size);
		}
		...
		{
			vasm+=(rf::vstr(rskey(c_mov64),rskey(c_rbx),rsoptr(c_comma))+
				ybase::trans_vword_to_vstr(src.vword));
		...
		const tdata* ptdi,rbuf<tasm>& vasm)
	{
		...
		vasm+=rf::vstr(rskey(c_sub),rskey(c_rsp),rsoptr(c_comma),size);
		vasm+=rf::vstr(rskey(c_mov64),rskey(c_rdi),rsoptr(c_comma),
			rskey(c_rsp));
		vasm+=rf::vstr(rskey(c_lea),rskey(c_rsi),rsoptr(c_comma),
			rsoptr(c_mbk_l),rskey(c_rbp),rsoptr(c_plus),
			rstr(ptdi->off),rsoptr(c_mbk_r));
		return add_rsi(sh,ptdi->type,vasm,src);
	}
		...
		const tsent& src,rbuf<tasm>& vasm)
	{
		...
		vasm+=rf::vstr(rskey(c_lea),rskey(c_rdi),rsoptr(c_comma),
			rsoptr(c_mbk_l),rskey(c_rsp),rsoptr(c_plus),
			rstr(size),rsoptr(c_mbk_r));
		vasm+=rf::vstr(rskey(c_mov64),rskey(c_rsi),rsoptr(c_comma),
			rskey(c_rsp));
		return add_rsi(sh,retval.type,vasm,src);
	}
	...
	// [ . , [ . , [ . , a , b ] , c ] , d ]
	static rbool add_rsi(const tsh& sh,rstr type,rbuf<tasm>& vasm,const tsent& src)
	{
		int start=0;
		...
		for(int i=0;i<start;i+=3)
		{
			...
			if(ptdi->off!=0)
			{
				vasm+=rf::vstr(rskey(c_add),rskey(c_rsi),
		...
		const rstr& dst,rbuf<tasm>& vasm)
	{
		...
		{
			vasm+=rf::vstr(rskey(c_mov64),rsoptr(c_mbk_l),
				rskey(c_rdi),rsoptr(c_mbk_r),
				rsoptr(c_comma),rsoptr(c_mbk_l),
				rskey(c_rsi),rsoptr(c_mbk_r));
			return true;
		}
		...
		{
			//源是对象
			vasm+=rf::vstr(rskey(c_push),rskey(c_rsi));
		}
		vasm+=rf::vstr(rskey(c_push),rskey(c_rdi));
